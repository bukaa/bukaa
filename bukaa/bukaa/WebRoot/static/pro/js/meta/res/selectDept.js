...
function initTree(){
	var setting = {
		async: {
			enable: true,
			url: urlpath + "meta/dept/getTreeData.sd",
...
function zTreeOnDblClick(event, treeId, treeNode) {
	...
	parent.$("input[name='resource.deptCode']").val(treeNode.code);
	parent.$("input[name='resource.deptName']").val(treeNode.name);
...
function selectDeptNode() {
	...
	if(treeNodes == 0) {
		...
		//top.layer.alert("请选择来源部门！", {icon: 0});
		return;
	}
...
function singlePath(newNode) {
    ...
    var zTree = $.fn.zTree.getZTreeObj("zymlTree"),
